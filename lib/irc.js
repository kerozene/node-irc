    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'MODE':
                ...
                chan = self.chanData(message.args[0]);
                if (!chan) break;
...
*/
            ...
            case 'PART':
                ...
                if (self.nick == message.nick) {
                    ...
                    self.emit('part', channel, message.args[1], message);
                    self.emit('part' + channel, message.args[1], message);
                    if (channel != channel.toLowerCase()) {
                        self.emit('part' + channel.toLowerCase(), message.args[1], message);
                    }
                }
            ...
            case 'KICK':
                ...
                channel = message.args[0];
                self.emit('kick', channel, message.args[1], message.nick, message.args[2], message);
                self.emit('kick' + channel, message.args[1], message.nick, message.args[2], message);
                if (channel != channel.toLowerCase()) {
                ...
                if (self.nick == message.args[1]) {
                    chan = self.chanData(channel);
                    delete self.chans[chan.key];
                    self.emit('kick', channel, message.nick, message.args[2], message);
                    self.emit('kick' + channel, message.nick, message.args[2], message);
                    if (channel != channel.toLowerCase()) {
                        self.emit('kick' + channel.toLowerCase(),
                    ...
                    chan = self.chanData(channel);
                    if (chan && chan.users) {
                        delete chan.users[message.args[1]];
                    }
                }
                break;
            case 'KILL':
            ...
            case 'QUIT':
                ...
                if (message.args[0] != 'Changing host') {
                    // TODO better way of finding what channels a user is in?
                    Object.keys(self.chans).forEach(function(channame) {
                        var channel = self.chans[channame];
                        delete channel.users[message.nick];
...
Client.prototype.who = function(target, format) {
    var sortFormat = this.who.format;
    format = format || '';
...
Client.prototype._addWhoDataToChan = function(target, data) {
    ...
    if (this.chanData(target)) {
        ...
        if (syncStart) {
            ...
            this.emit('syncchan', channel, syncTime);
            this.emit('syncchan' + channel, syncTime);
            if (this.opt.debug)
...
Client.prototype.haveChanMode = function(mode, channel) {
    return this.nickHasChanMode(self.nick, mode, channel);
};
...
Client.prototype.nickInChannels = function(nick) {
    var channels = [];
    _.each(this.chans, function(cdata, chan) {
        if (_.has(cdata.users, nick))
            channels.push(chan);
...
Client.prototype.nickIsInChannel = function(nick, channel) {
    var chan = self.chanData(channel);
    if (self._unknown(chan, 'channel', channel)) return false;
    return _.has(chan.users, nick);
};
...
Client.prototype.setChanMode = function(channel, mode, nicks) {
    ...
    var addsub = mode[0];
    mode = mode[1];
    if ( _.contains(['+', '-'], addsub) || !this.prefixForMode[mode] ) {
        this._unknown(undefined, 'mode', addsub + mode);
        return false;
    }
    nicks = _.chain(this.nickToUser(nicks))
                .pick(function(user) { return !this.userHasChanMode(user, mode); })
                .keys()
                .value();
    nicks = this._chunk(nicks, client.supported.modes); // number of modes allowed per line
