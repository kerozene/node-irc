            ...
            case 'MODE':
                ...
                modeList.forEach(function(mode) {
                    ...
                    if (mode in self.prefixForMode) {
                        ...
                        if (adding) {
                            if (channel.users[user] && !_.contains(channel.users[user]['modes'], self.prefixForMode[mode]))
                                channel.users[user]['modes'].push(self.prefixForMode[mode]);

                            self.emit('+mode', message.args[0], message.nick, mode, user, message);
                        }
                        else {
                            if (channel.users[user] && _.contains(channel.users[user]['modes'], self.prefixForMode[mode]))
                                channel.users[user]['modes'] = _.without(channel.users[user]['modes'], self.prefixForMode[mode]);
            ...
            case 'JOIN':
                ...
                if (self.nick == message.nick) {
                    ...
                    if (channel && channel.users) {
                        channel.users[message.nick] = { modes: [] };
                        self.send('WHO', message.nick, '%cuhsnfda');
                    }
                }
...
Client.prototype._addWhoData = function(data) {
    this._whoData.push(data);
};

Client.prototype._clearWhoData = function(target) {
    var data = this._whoData;
    this._whoData = [];
    return [target, data];
};

Client.prototype.who = function(target) {
    this.send('WHO', target, '%cuhsnfda');
};

Client.prototype._addWhoDataToChan = function(target, data) {
    var nick, status = [], user = {}, users = {};
        ...
        _.each(this.chans, function(cdata, chan) {
            _.each(users, function(data, nick) {
                if (_.has(cdata.users, nick)) {
                    ...
                    // WHO replies triggered by JOINs may arrive after MODE so omit modes
