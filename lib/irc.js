...
Client.prototype.nickHasChanMode = function(nick, mode, channel) {
    ...
    if (!chan) {
        if (this.opt.debug)
            util.log('nickHasChanMode: unknown channel: ' + channel);
        return false;
    }
    var modePrefix = this.prefixForMode[mode];
    if (!modePrefix) {
        if (this.opt.debug)
            util.log('nickHasChanMode: unknown mode: ' + mode);
        return false;
    }
    var user = chan.users[nick];
    if (!user) {
        if (this.opt.debug)
            util.log('nickHasChanMode: unknown nick: ' + nick);
        return false;
    }
...
Client.prototype.usersWithChanmode = function(mode, channel) {
    ...
    if (!chan)
        return {};
    mode = this.prefixForMode[mode];
    var modeUsers = _.filter(chan.users, function(user) {
        return ( user['modes'] && _.contains(user['modes'], mode) );
    });
    return modeUsers;
};
