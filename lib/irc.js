    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'NICK':
                ...
                channels = [];
                _.each(self.nickInChannels(newNick), function(channel) {
                    var chan = self.chans[channe];
                    chan.users[newNick] = chan.users[oldNick];
                    delete cdata.users[oldNick];
...
*/
            ...
            case 'KICK':
                ...
                if (self.nick == message.args[1]) {
                    ...
                    nickIsInChannel(nick, channel, 'remove');
                break;
            case 'KILL':
                nick = message.args[0];
                channels = [];
                _.each(nickInChannels(nick, 'remove'), function (channel) {
                    channels.push(channel);
                });
